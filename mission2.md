# PHÂN CÔNG NHIỆM VỤ REFACTOR MVC (2 NGƯỜI)

> Thành viên: 2 người
> Tổng số task: 24 (chia đều 12 task/người)
> Thời gian dự kiến: 2-3 ngày
> Reference: `refactor.md`

---

## THÀNH VIÊN A - INFRASTRUCTURE & AJAX LEAD

> Nhiệm vụ: Xây dựng nền tảng core + tất cả controllers có AJAX

### Task List (12 nhiệm vụ)
...
| 7  | `app/controllers/ComingSoonController.php` | `app/controllers/ComingSoonController.php` | `index()`, `subscribe()` AJAX                                     | Trung bình     |
| 8  | `index.php` modify                         | `index.php`                                | Bỏ switch-case, thêm `Router::dispatch()`                         | Cao             |
| 9  | View cleanup                                 | `app/views/contact/contact.php`            | Xóa dòng 7-145 (logic)                                              | Thấp           |
| 10 | View cleanup                                 | `app/views/about/teams.php`                | Xóa dòng 7-105 (logic)                                              | Thấp           |
| 11 | View cleanup                                 | `app/views/home/home.php`                  | Xóa dòng 19-108 (AJAX handling)                                     | Thấp           |
| 12 | View cleanup                                 | `app/views/comingsoon/comingsoon.php`      | Xóa dòng 12-14, 17-23 (logic)                                       | Thấp           |
...
- **Blocking:** Thành viên B chờ file này xong mới bắt đầu controllers
...
## THÀNH VIÊN B - CONTENT CONTROLLERS & HELPERS LEAD

> Nhiệm vụ: Tất cả content controllers + helpers + model fix + cleanup
...
| 8  | `core/helpers.php` modify                  | `core/helpers.php`                         | Thêm `format_date_vietnamese()`, `blogs_page_url()` | Thấp           |
| 9  | `app/models/ContactsModel.php` modify      | `app/models/ContactsModel.php`             | Dòng 55-56: bỏ `$_SERVER`, nhận từ parameter       | Thấp           |
| 10 | View cleanup                                 | `app/views/blogs/blogs.php`                | Xóa helper functions                                    | Thấp           |
| 11 | View cleanup                                 | `app/views/projects/projects.details.php`  | Xóa dòng 12-58 (model instantiation)                   | Thấp           |
| 12 | Testing & Review                             | -                                            | Test tất cả controllers, code review                   | -               |
...
- Xóa model instantiation từ view

**Task 12 (Testing & Review):**

- Test từng trang sau khi controllers xong
- Review code của A trước khi merge
...
| Buổi sáng  | **A** hoàn thành ValidationService + ContactController | A                   |
| Buổi chiều | **A** hoàn thành TeamsController + HomeController      | A                   |
| Buổi chiều | **B** bắt đầu BlogsController, ProjectsController     | B                   |
| Buổi chiều | **B** làm helpers (Task 8)                              | B                   |
...
| Giờ         | Hoạt động                                                  | Người phụ trách |
| ------------ | ------------------------------------------------------------- | ------------------- |
| Buổi sáng  | **Merge A vào refactor-2026** - B pull về             | All                 |
| Buổi sáng  | **A** hoàn thành ComingSoonController + cleanup views | A                   |
| Buổi sáng  | **B** tiếp tục controllers còn lại                  | B                   |
| Buổi sáng  | **B** làm model fix + cleanup views                    | B                   |
| Buổi chiều | **A** sửa index.php để dùng Router                  | A                   |
| Buổi chiều | **B** hoàn thành tất cả controllers                 | B                   |
| Buổi chiều | **B** testing & review                                  | B                   |
...
master (stable)
  ├── refactor-2026 (integration branch)
        ├── member-a/infrastructure  (Thành viên A)
        └── member-b/controllers     (Thành viên B)
```

### Quy tắc

1. **A tạo branch đầu tiên:** `member-a/infrastructure` từ `master`
2. **A merge vào refactor-2026** khi BaseController + Router xong
3. **B pull từ refactor-2026** để có BaseController
...
[A] Add ContactController with AJAX form handling
[B] Add BlogsController with filter and pagination
[B] Add helper functions for date formatting
```
...
| ---------------------- | ----------- | ------------------------------------------------------------------ |
| A chậm BaseController | Cao         | B có thể tạo controller skeleton không kế thừa, refactor sau |
...
- **Code review:** Pull request trên GitHub, review trước khi merge
...
| ------------ | --------------------------------- | -------- | --------------------- |
| **A**  | Infrastructure + AJAX Controllers | 12       | Cao                   |
| **B**  | Content Controllers + Helpers     | 12       | Trung bình           |
...
Khi cần đến auth và admin system:

- AuthController, UserController
- AuthService, UserService, AdminService
- Các trang login, register, dashboard, admin
