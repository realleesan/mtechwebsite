...
HTML;
    ...
    /**
     * Gửi email thông báo khi admin đăng nhập
     *
     * @param array $data ['email', 'ip', 'time']
     * @return array ['success' => bool, 'message' => string]
     */
    public function sendAdminLoginNotification($data)
    {
        try {
            $this->mailer->clearAddresses();
            ...
            if ($this->config['log_emails']) {
                error_log('EmailNotificationService: Admin login notification sent to ' . $data['email']);
            }

            return ['success' => true, 'message' => 'Email thông báo đã được gửi'];

        } catch (\PHPMailer\PHPMailer\Exception $e) {
            error_log('EmailNotificationService::sendAdminLoginNotification() - PHPMailer: ' . $e->getMessage());
            return ['success' => false, 'message' => 'Lỗi gửi email: ' . $e->getMessage()];
        } catch (\Exception $e) {
            error_log('EmailNotificationService::sendAdminLoginNotification() - General: ' . $e->getMessage());
            return ['success' => false, 'message' => 'Lỗi hệ thống: ' . $e->getMessage()];
        }
    }

    /**
     * Template email thông báo đăng nhập admin
     */
    private function getAdminLoginTemplate($data)
    {
        $email = htmlspecialchars($data['email']);
        $ip    = htmlspecialchars($data['ip'] ?? 'Unknown');
        $time  = htmlspecialchars($data['time'] ?? date('d/m/Y H:i:s'));
...
<head>
    ...
    <style>
        ...
        .container { max-width: 560px; margin: 0 auto; background: #fff; border-radius: 10px; overflow: hidden; box-shadow: 0 4px 16px rgba(0,0,0,0.1); }
        .header { background: #1A3FBF; color: #fff; padding: 28px 24px; text-align: center; }
        .header h1 { margin: 0; font-size: 22px; }
        .header p { margin: 6px 0 0; opacity: 0.85; font-size: 14px; }
        .body { padding: 28px 24px; }
        .alert-box { background: #fff8e1; border-left: 4px solid #ffc107; padding: 14px 16px; border-radius: 4px; margin-bottom: 20px; }
        ...
        .info-table td:first-child { font-weight: 600; color: #444; width: 35%; }
        .btn-block { text-align: center; margin-top: 24px; }
        .btn { display: inline-block; background: #1A3FBF; color: #fff; padding: 12px 28px; border-radius: 6px; text-decoration: none; font-weight: 600; font-size: 14px; }
...
<body>
    <div class="container">
        ...
        <div class="body">
            ...
            <p style="color:#555; font-size:14px;">Một phiên đăng nhập mới vừa được thực hiện với thông tin sau:</p>
            ...
            <div class="btn-block">
                <a href="https://admin.mtechjsc.com" class="btn">Truy cập Admin Panel</a>
            </div>
        </div>
        <div class="footer">
...
<body>
    <div class="container">
        <div class="header">
            <img src="https://admin.mtechjsc.com/assets/images/logo.png" alt="MTECH.JSC">
